...
LABEL org.label-schema.name="kafka" \
      ...
      org.label-schema.vcs-url="https://github.com/giangtong-katalon/kafka-docker" \
      org.label-schema.vcs-ref="${vcs_ref}" \
      org.label-schema.version="${scala_version}_${kafka_version}" \
      org.label-schema.schema-version="1.0" \
      maintainer="giangtong"
...
RUN set -eux ; \
    apt-get update ; \
    apt-get upgrade -y ; \
    apt-get install -y --no-install-recommends wget curl jq net-tools ; \
    chmod a+x /tmp2/*.sh ; \
